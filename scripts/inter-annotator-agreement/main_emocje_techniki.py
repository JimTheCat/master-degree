...
dla adnotacji emocji i technik retorycznych.
======================================================================
Porównuje dwa pliki CSV z adnotacjami w formacie:
    id;kategorie
    ID;{"emocje": [...], "techniki_retoryczne": [...]}
...
Użycie:
    python main_emocje_techniki.py plik_annotator1.csv plik_annotator2.csv
"""
...
def load_annotations(filepath: str) -> dict[str, dict]:
    """Wczytuje plik CSV i zwraca dict: id → {'emocje': set, 'techniki_retoryczne': set}."""
    annotations = {}
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            if len(row) < 2 or not row[0].strip():
                continue
            uid = row[0].strip()
            raw_json = ";".join(row[1:]).strip()  # na wypadek gdyby JSON zawierał ';'
            try:
                data = json.loads(raw_json)
            except json.JSONDecodeError:
                print(f"  ⚠ Nie udało się sparsować JSON dla id={uid}: {raw_json[:80]}")
                continue
            annotations[uid] = {
                "emocje": set(data.get("emocje", [])),
                "techniki_retoryczne": set(data.get("techniki_retoryczne", [])),
            }
    return annotations
...
def build_binary_vectors(ann1: dict, ann2: dict, category: str):
    """
    Dla danej kategorii ('emocje' lub 'techniki_retoryczne') tworzy
    binarne wektory per etykieta.
    ...
    for uid in common_ids:
        all_labels |= ann1[uid][category]
        all_labels |= ann2[uid][category]

    vectors = {}
    for label in sorted(all_labels):
        v1 = [1 if label in ann1[uid][category] else 0 for uid in common_ids]
        v2 = [1 if label in ann2[uid][category] else 0 for uid in common_ids]
        vectors[label] = (v1, v2)

    return vectors, common_ids


def find_disagreements(ann1, ann2, common_ids, category):
    """Zwraca listę (id, tylko_w_ann1, tylko_w_ann2) gdzie wystąpiły rozbieżności."""
    disagreements = []
    for uid in common_ids:
        s1 = ann1[uid][category]
        s2 = ann2[uid][category]
        if s1 != s2:
...
def main():
    if len(sys.argv) < 3:
        print("Użycie: python inter_annotator_agreement.py <plik1.csv> <plik2.csv>")
        sys.exit(1)
    ...
    for category_name, category_key in [
        ("EMOCJE", "emocje"),
        ("TECHNIKI RETORYCZNE", "techniki_retoryczne"),
    ]:
        print(f"{'─'*72}")
        print(f"  Kategoria: {category_name}")
        print(f"{'─'*72}")

        vectors, common_ids = build_binary_vectors(ann1, ann2, category_key)
        ...
        avg_p = sum(agreements) / len(agreements)
        ...
        # Rozbieżności
        disagreements = find_disagreements(ann1, ann2, sorted(common), category_key)
        if disagreements:
    ...
    # ---------------------------------------------------------------------------
    out_path = "rozbieznosci.csv"
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f, delimiter=";")
        w.writerow(["id", "kategoria", "annotator_1", "annotator_2",
                     "tylko_annotator_1", "tylko_annotator_2"])
        for category_key in ["emocje", "techniki_retoryczne"]:
            for uid in sorted(common):
                s1 = ann1[uid][category_key]
                s2 = ann2[uid][category_key]
                if s1 != s2:
                    w.writerow([
                        uid,
                        category_key,
