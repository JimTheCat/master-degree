...
def render_category_selector():
    ...
    st.markdown("### 🎭 GRUPA 1: EMOCJE I POSTAWY AFEKTYWNE")
    with st.expander("ℹ️ Opisy kategorii emocji", expanded=False):
        st.markdown("""
        - **AGRESJA_WERBALNA**: Bezpośrednia ekspresja złości, gniewu, oburzenia. Inwektywy, krzyk, wulgaryzmy.
        - **STRATEGIA_STRACHU**: Wyrażanie lęku lub celowe straszenie odbiorców zagrożeniem (wojną, biedą, zniszczeniem państwa).
        - **DEHUMANIZACJA_POGARDA**: Wyrażanie wyższości, obrzydzenia, traktowanie oponentów jako "podludzi", "zarazę" lub "szkodników".
        - **DUMA_I_SUKCES**: Afirmacja własnej grupy. Wyrażanie satysfakcji, dumy z osiągnięć, siły i sprawczości.
        - **MESJANIZM_MORALNY**: Poczucie moralnej wyższości i misji dziejowej. Własna grupa jako jedyny obrońca Dobra/Prawdy.
        """)
    ...
    st.markdown("### 🎯 GRUPA 2: TECHNIKI RETORYCZNE")
    with st.expander("ℹ️ Opisy technik retorycznych", expanded=False):
        st.markdown("""
        - **POLARYZACJA_MY_ONI**: Fundamentalny podział na dwa zwaśnione obozy ("My"-dobrzy vs "Oni"-źli).
        - **AD_HOMINEM**: Atak na osobę (wygląd, przeszłość, cechy), a nie na jej argumenty.
        - **OBLĘŻONA_TWIERDZA**: Przedstawianie własnej grupy jako ofiary niesprawiedliwych ataków, nagonki lub spisku.
        - **PRZYPISYWANIE_ZŁYCH_INTENCJI**: Sugerowanie, że przeciwnik działa celowo na szkodę (agentura, zdrada, tajny plan).
        - **WHATABOUTISM**: Odwracanie uwagi poprzez wytykanie hipokryzji lub błędów z przeszłości.
        - **SOFIZMAT_ROZSZERZENIA**: (Straw man) Atakowanie przeinaczonej, wyolbrzymionej wersji poglądów przeciwnika.
        - **DOWÓD_ANEGDOTYCZNY**: Używanie pojedynczych historii/plotek przeciwko twardym danym. Populizm.
        - **APEL_O_JEDNOŚĆ**: Wzywanie do zgody (często instrumentalne, by uciszyć krytykę).
        """)
